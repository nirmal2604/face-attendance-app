...
def retrive_data(name):
    ...
    # Convert bytes to np arrays
    retrive_series = retrive_series.apply(lambda x: np.frombuffer(x, dtype=np.float32))

    # Decode keys
    index = [k.decode() for k in retrive_series.index]
    ...
    # ✅ Filter valid keys: must contain exactly one '@'
    valid_mask = retrive_df['name_role'].str.count('@') == 1
    if not valid_mask.all():
        bad_keys = retrive_df.loc[~valid_mask, 'name_role'].tolist()
        print("⚠️ Skipping invalid Redis keys:", bad_keys)

    retrive_df = retrive_df[valid_mask].copy()

    # ✅ Split safely into Name and Role
